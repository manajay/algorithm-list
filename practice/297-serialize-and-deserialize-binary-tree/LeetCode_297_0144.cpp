...
/// 超内存限制, 主要是 同时也处理了 空节点,所以才这样, 优化处理 https://leetcode-cn.com/problems/serialize-and-deserialize-binary-tree/solution/cchao-guo-9927-by-noble-monster/, 不处理空指针, 采用标记的时候处理层级标记
class Codec
{
public:
    ...
    string serialize(TreeNode *root)
    {
        ...
        q.push(root);
        TreeNode *cur = NULL;
        bool levelEmpty = false;
        int count = 1;
        while (!q.empty() && !levelEmpty)
        {
            // 标记当前层是否有非null节点
            levelEmpty = true;
            int curCount = 0;
    ...
    TreeNode *deserialize(string data)
    {
        ...
        TreeNode *cur = NULL, *preNode = NULL;
        TreeNode *root = NULL;
        int times = 0;
...
public:
    ...
    TreeNode *deserialize(string data)
    {
        ...
        root = cur;
        // q 内元素 不需要使用 pair 额外添加是否构建lleft right的标记,
        // 我们只需要每次读取两个数值即可
        q.push(cur);
        while (true)
        {
